...
test('test simple index', function (t) {
  ...
  levelup(location, function (err, db) {
    ...
    var cb = after(4, delayed(function (err) {
      var i = 0, j = 0

      t.notOk(err, 'no error')

      db.getBy('key', '1', function (err, values, keys) {
        t.notOk(err, 'no error')

        t.deepEqual(values, [
            '{"one":"ONE","key":"1"}'
        ], 'correct values')
        t.deepEqual(keys, [ 'foo1' ], 'correct keys')

        end()
      })

      db.getBy('bleh', 'true', function (err, values, keys) {
        t.notOk(err, 'no error')
        ...
        t.deepEqual(keys, [ 'foo2', 'foo3' ], 'correct keys')
